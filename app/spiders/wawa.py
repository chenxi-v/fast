...
class Spider(Spider):
    ...
    def detailContent(self, ids):
        ...
        type_name = None
        try:
            types_data = self.fetch(f"{self.host}/api.php/zjv6.vod/types", headers=self.getheader()).json()
            for item in types_data['data']['list']:
                if item['type_id'] == vod['type_id']:
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        try:
            ...
            # 检查URL是否是视频格式
            if self.isVideoFormat(target_url):
                return {
                    'parse': 0,
                    'url': target_url,
                    'header': {'User-Agent': 'dart:io'}
